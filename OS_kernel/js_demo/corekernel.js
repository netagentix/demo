...
// Robust process shim for environments (browser/Node)
if (typeof globalThis.process === 'undefined') {
  globalThis.process = { env: {} };
}
// Provide minimal hrtime API for browsers lacking Node's process.hrtime
if (typeof globalThis.process.hrtime === 'undefined') {
  ...
  globalThis.process.hrtime = function (prev) {
    ...
    if (prev && Array.isArray(prev)) {
      const diffSec = sec - prev[0];
      const diffNs = ns - prev[1];
      return [diffSec, diffNs];
    }
    return [sec, ns];
  };
  // bigint version for high-resolution timing
...
export { Vector, Semantic, AgentixKernel };
